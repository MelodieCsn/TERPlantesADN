...
```
Ici, nous remarquons que les éléments dont la distribution ne suit pas une loi normale après avoir enlevé les outliers sont : Cu, Fe et C
...
i = 2
while (i < 17) {
  print(paste("Test de Student entre les données", names(lang_no_out[i]), "et", names(lang_no_out[i+1])))
  print(paste("p-value = ",t.test(lang_no_out[,i+1], lang_no_out[,i], paired = TRUE)$p.value))
  i = i + 2
}
```
...
#lang_no_out_no_Mg <- subset (lang_no_out, select = -c(Mg_eCo2,Mg_aCo2))
#i = 2
#while (i < 15) {
  #print(paste("Test de Fligner-Kileen's entre les données", names(lang_no_out[i]), "et", names(lang_no_out[i+1])))
  #print(paste("p-value = ",fligner.test(lang_no_out_no_Mg[,i+1], lang_no_out_no_Mg[,i], paired = TRUE)$p.value))
  #i = i + 2
#}
```

Ce test  nous renvoie que les données sont essentiellement constantes
