...
acpcsv <- read.csv("regmap_dim1.csv",sep = "\t")

#On veut ajouter à regmap la colonne comp1 avec les valeurs de l'acp pour les plantes 
#qui ont le même nom que la colonne de nom à gauche de regmap

rownames(acpcsv) <- acpcsv$Genotypes
regmap$Comp1 <- NA
regmap[rownames(acpcsv),"Comp1"] <- acpcsv$Dim.1
...
Y <- regmap[match(acc, regmap$ecotype),stringr::str_detect(colnames(regmap), "change|Comp")]
Y$genotype <- acc
...
write.table(t, "t50Comp1Astle.csv", row.names=FALSE, sep=",")

# peut prendre du temps
plot(gwas, plotType = "manhattan", trait = "Comp1", lod = 3)
