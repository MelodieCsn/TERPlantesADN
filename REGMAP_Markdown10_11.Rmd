...
i = 2
while (i < 17) {
  print(paste("Test de Shapiro entre les données", names(reg_no_out[i]), "et", names(reg_no_out[i+1])))
  print(shapiro.test(reg_no_out[,i])$p.value)
  i = i + 2
}
```
Ici, nous remarquons que les éléments dont la distribution ne suit pas une loi normale après avoir enlevé les outliers sont : C, Zn, Na, Mg et Cu
...
i = 2
while (i < 17) {
  print(paste("Test de Student entre les données", names(reg_no_out[i]), "et", names(reg_no_out[i+1])))
  print(paste("p-value = ",t.test(reg_no_out[,i+1], reg_no_out[,i], paired = TRUE)$p.value))
  i = i + 2
}
```
