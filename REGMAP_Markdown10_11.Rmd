...
```{r}
reg_no_out_ch <- select(reg_no_out_ch,"genotype"|contains("change"))
```
...
fviz_nbclust(reg_no_out_ch[,2:9], FUNcluster = kmeans, method = "gap_stat")
...
lis = c(1,10,25,50)
for (i in lis)  {
  clusters <- kmeans(reg_no_out_ch[,2:9],centers = 2, nstart = i )
  graph <- fviz_cluster(clusters, data = reg_no_out_ch[,2:9], 
             ellipse.type = "convex",
             palette = "jco",
             repel = TRUE,
...
```{r}
cluster <- kmeans(reg_no_out_ch[,2:9],centers = 6, nstart = 25 )
graph <- fviz_cluster(cluster, data = reg_no_out_ch[,2:9], 
             ellipse.type = "convex",
             palette = "jco",
             repel = TRUE,
             ggtheme = theme_minimal())
...
```
```{r}
cluster_pam <- pam(reg_no_out_ch[,2:9], k = 2)
fviz_cluster(cluster_pam, data = reg_no_out_ch[,2:9], 
             ellipse.type = "convex",
             palette = "jco",
             repel = TRUE,
             ggtheme = theme_minimal())
